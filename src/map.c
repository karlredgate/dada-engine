...
pMapAction
new_subst_action(char *key, char *out) {
    pMapAction r = (pMapAction) malloc(sizeof(MapAction));
    r->mode = subst;
    r->key = key;
    r->out = out;
...
pMapOpt
mapopt_concat(pMapOpt a, pMapOpt b) {
    pMapOpt r = a;
    while (a->next)
	a = a->next;
    a->next = b;
...
pMapOpt
mapopt_cons(char *key, pMapAction act, pMapOpt cdr) {
...
pMapOpt
mapopt_match(pMapOpt list, char *string) {
    while (list) {
	regex_t key_regex;
	regmatch_t match;
...
pMapping
map_cons(char *n, pMapOpt opts, pMapping cdr) {
...
pMapping
map_lookup(pMapping list, char *name) {
...
char *
regex_subst(char *in, char *from, char *to) {
    char *r;
    regex_t re;
    regmatch_t match;
    int len;
...
char *
apply_mapping(char *input, pMapping m) {
    pMapOpt option;
    ...
    if (!option)
	return input;		/* by default, let it be */
