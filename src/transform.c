...
pTransformation
trans_lookup(pListNode list, char *name) {
    return (pTransformation) ((list) ? ((strcmp
					 (((pTransformation) list->data)->
					  name,
					  name) ==
					 0) ? list->
					data : trans_lookup(list->next,
							    name)) : NULL);
}
...
pTransOpt
transopt_match(pListNode list, char *string) {
    while (list) {
	regex_t key_regex;
	regmatch_t match;
...
static inline int
addr_normalise(int addr, char *input) {
    int l = strlen(input);
    if (l == 0)
	return 0;
    while (addr < 0)
...
static char *
apply_xform_cmd(char *input, pTransCmd cmd) {
    int from, to, len, i;
    char *r;
    ...
    case 'd':
	r = malloc(strlen(input) - len);
	*r = '\0';
	strncpy(r, input, from);
	strncat(r, input + to + 1, strlen(input) - (to + 1));
	free(input);
	return r;
...
char *
apply_xform(char *input, pTransformation xfm) {
    pTransOpt option;
    pTransCmd cmd;
