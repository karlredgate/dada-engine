...
static inline pTransCmd
new_transcmd(int *addr, char *id, pListNode args) {
    pTransCmd r = (pTransCmd) malloc(sizeof(TransCmd));
    r->addr = addr;
    strncpy(r->cmdname, id, CMDNAMELENGTH - 1);
    free(id);			/* we can do this */
    r->params = args;
    r->next = NULL;
    return r;
}

static inline pTransOpt
new_transopt(char *key, pTransCmd cmds) {
...
static inline pTransformation
new_transformation(char *name, pListNode opts) {
