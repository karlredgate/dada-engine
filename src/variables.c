...
new_str_var(char *name, char *val) {
...
static struct var *
var_insert_str(struct var *tree, char *name, char *val) {
    if (tree) {
...
static struct var *
var_insert_int(struct var *tree, char *name, int val) {
    if (tree) {
	if (strcmp(name, tree->name) == 0) {
	    tree->type = int_t;
	    tree->value.i = val;
...
void
var_put(char *var_name, char *value) {
